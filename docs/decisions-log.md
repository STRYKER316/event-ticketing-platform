...
**Amendment (Phase 7, 2026-08-20):** the "live status from Booking Service's Ticket rows" half of this decision never had a read route to actually fetch through — `booking-service` exposed only `POST /bookings`, `POST /bookings/{id}/pay`, and `POST /bookings/{id}/cancel` before Phase 7. Resolved before P7.T1 began (same practice as the §7.2/§9/§16/§22 amendments): a new **public, no-auth** `GET /bookings/events/{event_id}/tickets` on Booking Service, returning `[{ticket_id, section, row_name, seat_label, status, price_cents}]` — one row per ticket, joined client-side against Event Service's seat-map layout on `(section, row_name, seat_label)`. Public rather than authenticated because browsing seat availability shouldn't require login, matching `event-service`'s own public `GET /events*` routes — only the booking action itself (already authenticated) requires one. Implemented as a thin `TicketRepository.list_by_event` query plus a route with no Manager class, since it's a pure read with no business rule — the same "no equivalent write path to unify with, so talk to the Repository directly" shape `search-service`'s `EventConsumer` already established for the analogous case.
