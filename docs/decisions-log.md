...
**Benchmark plan:** concurrent clients against the same small seat pool (k6 or a multi-threaded harness — resolved P8.T2, 2026-08-16: a standalone Python `asyncio` harness, `/benchmark`, reusing the `asyncio.gather` pattern already proven in P3.T7; no decisions-log entry needed for this specific choice per `docs/phases/phase-8-kickoff.md`'s own note). Metrics: successful/failed bookings, hold-acquisition latency, time-to-release-after-abandonment. This is the centerpiece of the report's Feature Development Process chapter — real measured numbers only, never fabricated. **Measured, P8 (n=3 runs per strategy):** see §17's amendment below for the release-latency numbers, and `docs/report/feature-development-process.md` for the full comparison — hold-acquisition and passive-release latency are statistically indistinguishable between the two strategies at this benchmark's scale; the one consistent difference is immediate-release trigger time, where cron holds a small (millisecond-scale) edge in all three runs.
...
**Amendment (Phase 4, 2026-08-17):** point #4 originally described only the failure path ("payment declined/failed"). Broadened at implementation time to a single topic carrying both `succeeded` and `failed` outcome messages (an `action`-style field, mirroring the `UPSERTED`/`DELETED` pattern point #2 already uses on `event.events`) rather than adding a sixth integration point for the success case, which would have exceeded the five-point cap this section states — Booking Service's consumer for this topic handles both `PENDING` → `CONFIRMED` (success, §21) and `PENDING` → `EXPIRED` + immediate hold release (failure, the original description, unchanged in mechanism). This keeps what is really one Payment→Booking relationship (payment outcome) with two directions as one integration point, not two.
...
**Amendment (Phase 4, 2026-08-17):** two gaps surfaced when Payment Service moved from decision to implementation, both flagged as open questions and resolved before P4.T1 began (same practice as the §7.2/§15 amendments — surface an architecture fork rather than silently guess at it):

1. **Ticket pricing** — no price field existed anywhere in the system (Event, seat map, or Ticket) for Payment Service to charge against. Resolved: **per-section pricing**, set by the organizer as part of the seat map (`SeatMapSection.price_cents`, Event Service — see §16 amendment). Carried through the existing event-carried Kafka payload (§7.2 — `EventSeat.price_cents` added to `EventUpsertedMessage`) into Booking Service's `Ticket.price_cents` at provisioning time (retroactively extends P3's `ProvisioningConsumer`/`bulk_upsert_available`). Chosen over a flat per-event or hardcoded global price for realism against the reserved/numbered seating model (§16) — floor-vs-balcony pricing is the point of numbered sections.

2. **Ownership verification without a synchronous query into `booking_db`** — Payment Service has no access to `booking_db` (§8) to check that the paying user actually owns the booking being charged. Resolved: **Booking Service fronts payment.** The client calls a new ownership-scoped `POST /bookings/{id}/pay` on Booking Service (which already owns and can check the booking, same ownership-scoping convention as every other endpoint per `CLAUDE.md`), valid only on a `PENDING` booking. Booking Service then makes a synchronous call to Payment Service's charge endpoint, forwarding the caller's JWT unmodified (Payment Service validates it the normal way via `shared_auth.get_current_user` — no separate service-to-service auth scheme needed) and passing the ticket's `price_cents`, which Booking Service already holds locally. This is the system's first synchronous inter-service call — a deliberate, narrow exception to "cross-service data only arrives via Kafka" (`CLAUDE.md`'s architecture invariants), justified because initiating a charge is a request/response action needing an immediate result (did the attempt succeed or fail, right now), not a fact to propagate eventually, which is what the five Kafka integration points (§7) are for elsewhere in this system. The actual success/failure *confirmation* stays Kafka-driven and webhook-sourced (§7 point #4, broadened above) — this synchronous call only initiates the charge attempt, it does not confirm it.
...
- The frontend's three backend service URLs and the Keycloak OIDC issuer were baked-in `http://localhost` build-time constants; Keycloak's realm `redirectUris`/`webOrigins` were hardcoded the same way. Fixed: the three service URLs are now relative (Traefik proxies both from the same origin), the Keycloak issuer is resolved at runtime from `window.location` (Keycloak isn't proxied through Traefik, so it can't be relative), and the realm config is rendered from a template by a Keycloak entrypoint script driven by an `APP_ORIGIN` env value.
- PKCE's `code_challenge` needs `crypto.subtle`, which browsers restrict to secure contexts (HTTPS, or a `localhost` hostname specifically) — worked in every local test, failed silently against the EB deployment's plain-HTTP real hostname. Fixed with a pure-JS SHA-256 fallback, keeping PKCE at full S256 strength rather than downgrading to `plain`.
- `infra/docker-compose.yml`'s build contexts (`../services`, `../frontend`) reach outside `infra/`, which EB's Docker-Compose deploy can't resolve (it expects the compose file at the deployment bundle's root with every context nested underneath). Fixed with `infra/build-eb-bundle.sh`, which assembles a flat, self-contained bundle without touching the canonical compose file used for local dev. EB also has no host-side Python/uv, so a `.platform/hooks/postdeploy` script now execs into each service's own container to run the same migrate+seed steps `make migrate`/`make seed` run locally.

**Credential model (durable summary; full reasoning in the kickoff doc's "Credential & execution model" section):** an IAM user (`anshilM`, not root) with `AdministratorAccess` — attached via an `admin` IAM group, not directly on the user — is what this project authenticates as for every AWS-touching command. Broader than this phase strictly needs (the minimum would be `AdministratorAccess-AWSElasticBeanstalk` + a narrow Budgets permission), a deliberate trade-off for a solo capstone. Auth flow actually used: `aws login` (AWS CLI v2.32+, temporary console-session credentials, auto-refreshing, no static key ever written to disk) rather than `aws configure`'s long-lived access key/secret — strictly safer than the originally-planned flow, not a downgrade. Region: `ap-south-1`, a deliberate choice, not `us-east-1` — the cost figures elsewhere in this section and in §13 were priced for `us-east-1` and reconciled against real `ap-south-1` billing at P10.T4.
...
**Amendment (Phase 4, 2026-08-17):** each section carries a `price_cents` set by the organizer as part of the seat map (§9 amendment) — per-section pricing (e.g. floor vs. balcony), not a flat price per event or per system.
...
**Amendment (Phase 6, 2026-08-17):** three gaps surfaced when this section moved from decision to implementation, all flagged as open questions and resolved before P6.T1 began (same practice as the §7.2/§9 amendments — surface an architecture fork rather than silently guess at it):

1. **"Reusing the exact same release mechanism" is not literally true at the code level.** `TicketHoldStrategy.release_hold()` (§6) only transitions a ticket `HELD` → `AVAILABLE` (`CronHoldStrategy`'s conditional `UPDATE ... WHERE status = 'HELD'`) — a `CONFIRMED` booking's ticket is `BOOKED`, not `HELD`, so calling `release_hold()` on it is a silent no-op under the cron strategy, which would leave a cancelled seat permanently unbookable (`_fetch_bookable_ticket` in `BookingManager` rejects any ticket whose status is `BOOKED`). Resolved: a new `TicketHoldStrategy.release_booking(ticket_id)` method, added to the ABC alongside `acquire_hold`/`release_hold`/`confirm_hold`. `CronHoldStrategy` implements it as `UPDATE tickets SET status = 'AVAILABLE' WHERE id = ? AND status = 'BOOKED'` — same rowcount-gated conditional-UPDATE shape as its siblings. `RedisHoldStrategy` implements it as a no-op: that strategy never writes `tickets.status` at all (documented in its own class docstring since §6/P3) — a `CONFIRMED` booking's ticket is already sitting at `AVAILABLE` under Redis, and correctness against re-booking an active booking's ticket comes from `uq_bookings_active_ticket` (the partial unique index scoped to `PENDING`/`CONFIRMED` bookings), which stops blocking the instant this cancellation flips `Booking.status` to `CANCELLED` — no Ticket-table write needed under Redis, same asymmetry the hold-acquisition mechanism already has. `FakeHoldStrategy` gets a matching trivial implementation for the unit-test suite. This is a genuine third method, not a `release_hold()` reuse — the "same mechanism" framing above refers to the general *shape* (an idempotent, rowcount-gated `TicketHoldStrategy` method reused by both `BookingManager` and any Kafka consumer that needs it), not a literal single method call.
2. **The "before the event's start time" cutoff has nowhere to read a start time from.** Booking Service's own tables (`Ticket`, `Booking`) never store event start time — the only place it exists in `booking_db`'s world is a field on the Kafka message that provisions tickets (`EventUpsertedMessage.start_time`, already received and then discarded by `ProvisioningConsumer`). Resolved the same way P4.T1 resolved an identical "the data exists on a message we already receive but nowhere in our schema" gap for pricing: add a small `Event(event_id PK, start_time)` reference table to `booking_db`, written by `ProvisioningConsumer` alongside ticket provisioning (same message, no new Kafka point), and have `BookingManager.cancel_booking` reject (409) a cancellation attempt at or after that timestamp. This is genuinely retroactive scope on P6.T1, same shape as P4.T1's own retroactive touch — done there rather than silently leaving §22's stated cancellation policy unenforced, which the Integrity rule doesn't allow describing as decided-and-working if it isn't actually checked anywhere.
3. **Kafka #3's consumer (Notification Service) doesn't exist yet at this point in the locked report-first build order** (§27: P6 runs before P5). Resolved: this is not actually a blocker, because Kafka producer and consumer are independently deployable — Payment Service can publish to the shared `notifications` topic now, and Phase 5 building the consumer later is ordinary eventual-consistency, the same trade-off already accepted for point #2 (§7's "documented trade-off, not a bug" note). P6.T3 therefore builds the **producer** side only: on a refund failure, Payment Service publishes a `NotificationMessage` (`action: refund_failed`, `booking_id`, `reason`) to `notifications`. P6's own validation checkpoint verifies delivery with a throwaway/console consumer, not Notification Service's actual retry/DLQ pipeline (§17), which doesn't exist until P5. The other two notification triggers this integration point eventually carries — booking-confirmed (owned by `PaymentOutcomeConsumer` in Booking Service, §7 point #4) and payment-confirmed (owned by `PaymentManager.handle_webhook_event` in Payment Service, §9) — are deliberately **not** retrofitted in P6; both are out of P6's scope (cancellation/refund only) and are deferred to Phase 5 itself, added alongside the consumer that will finally read them, per P5.T2's own task description in `master-development-plan.md`.
...
**Amendment (Phase 7, 2026-08-20):** the "live status from Booking Service's Ticket rows" half of this decision never had a read route to actually fetch through — `booking-service` exposed only `POST /bookings`, `POST /bookings/{id}/pay`, and `POST /bookings/{id}/cancel` before Phase 7. Resolved before P7.T1 began (same practice as the §7.2/§9/§16/§22 amendments): a new **public, no-auth** `GET /bookings/events/{event_id}/tickets` on Booking Service, returning `[{ticket_id, section, row_name, seat_label, status, price_cents}]` — one row per ticket, joined client-side against Event Service's seat-map layout on `(section, row_name, seat_label)`. Public rather than authenticated because browsing seat availability shouldn't require login, matching `event-service`'s own public `GET /events*` routes — only the booking action itself (already authenticated) requires one.

**This corrects an earlier version of this amendment**, which described the route as built with no `BookingManager` method ("a pure read with no business rule... talk to the Repository directly," the same shape `search-service`'s Kafka-only `EventConsumer` uses). That reasoning doesn't actually hold here: this is an API route, not a Kafka consumer, and CLAUDE.md's layering rule only carves out the Repository-direct exception for a consumer with no equivalent API route to unify with — this route *is* the equivalent entry point, so the general rule (every route calls a Manager) applies. Found and fixed in the phase's `/pre-pr` code-review pass: the route now calls `BookingManager.list_tickets_for_event(event_id)`, which does the same `TicketRepository.list_by_event` query and DTO mapping the route used to do inline.
...
- No confirmation-page refresh persistence — there is no `GET /bookings/{id}` route, so `ConfirmationPage` renders only from the in-memory state a successful checkout hands off via React Router; a hard refresh loses the displayed detail even though the underlying booking is unaffected and already `CONFIRMED` server-side — §10, Phase 7
...
- `HOLD_STRATEGY` is assumed fixed for a deployment's whole lifetime, not safely switchable with in-flight bookings outstanding — a booking confirmed under `cron` and later cancelled after a live switch to `redis` would leave its ticket stuck `BOOKED` (`RedisHoldStrategy.release_booking` is a documented no-op, §6/§22), permanently unbookable. Found in Phase 6 code review; not fixed, since a real fix would mean `RedisHoldStrategy` writing `tickets.status` after all, undoing the whole point of that strategy's design (§6). `HOLD_STRATEGY` was never switched live against real state anywhere in this system — P8's benchmark only ever flips it between runs against a fresh seat pool — so this is a pre-existing boundary of the whole dual-strategy design, not something Phase 6 introduced; Phase 6 is just the first place a consequence of it became concrete.
- `RedisHoldStrategy.confirm_hold`/`release_hold` are not transactional with the Postgres write in the same logical operation (a plain `redis.delete`, not participating in the SQLAlchemy session's commit/rollback) — a DB-side rollback of that operation (a retried-then-abandoned write, per the bounded-retry-then-give-up pattern every Kafka consumer with a side effect in this system uses) can leave the Redis key deleted while the corresponding Postgres row reverts, a narrow inconsistency window under the `redis` strategy specifically. Found in Phase 5 code review, not fixed — a real fix means either a two-phase-commit-style protocol Postgres and Redis don't share, or moving the Redis operation inside the same retry unit in a way that risks re-deleting an already-deleted key (itself a no-op, so likely safe, but not exercised or proven this phase). Pre-existing since Phase 3's original `RedisHoldStrategy` design, not introduced by Phase 5 — Phase 5 is just the first place a call site (`PaymentOutcomeConsumer`) made the compounding failure mode (DB rollback after a Redis side effect already fired) concrete enough to flag.
- A background Kafka consumer task that dies (its `run()` loop's exception escapes uncaught) is only logged (`critical`, via each service's `_log_if_died` callback) — nothing restarts it, and no `/healthz` check reflects that it's down, across all three services with consumer tasks (`booking-service`, `payment-service`, `notification-service`). A crashed consumer silently stops processing its topic until the container itself is restarted. A deliberate scope boundary rather than an oversight in each individual service (bounded per-message retries inside the handler are what's meant to keep a task alive under transient failures — see `DB_WRITE_MAX_ATTEMPTS`/`_run_with_retry` and, as of Phase 5, `PUBLISH_MAX_ATTEMPTS`/`_publish_with_retry`), but genuine process-level supervision (a container orchestrator restarting a crashed process, which Docker Compose alone doesn't do without a `restart` policy tied to an actual health signal) was never built.
- `booking-service`'s `ProvisioningConsumer`/`PaymentOutcomeConsumer`: when `_run_with_retry`'s bounded DB-write retries exhaust, the handler logs `critical` and returns without raising, and the manual-commit consume loop advances the offset regardless — the message is permanently dropped with no secondary topic or replay mechanism, unlike `notification-service`'s dedicated retry/DLQ ladder (§17). Only reachable under a Postgres outage lasting longer than the bounded retry window at the exact moment a message is processed. Found in a pre-Phase-10 hardening/testing pass; not fixed, since building a DLQ for booking-service's consumers is new architecture, not a bug fix — same reasoning as the crash-recovery limitation above.
- A narrower race in the same two consumers: if a DB commit succeeds server-side but the client-side `await` raises (e.g. a transient network blip), `_run_with_retry` re-runs the operation in a fresh session, finds the row already in its target state, and treats that as a no-op — the booking itself ends up correctly confirmed, but the one-time booking-confirmed notification tied to that code path is silently skipped. Found in the same pass; not fixed, since a real fix needs idempotent notification-delivery tracking this system doesn't otherwise have.
- `notification-service`'s `RetryConsumer` processes the `notification-retry` topic strictly serially — one booking's backoff delay blocks every other booking's retry queued behind it on the same partition, so a burst of concurrent failures degrades retry throughput under exactly the load the retry ladder exists to handle. Found in the same pass; not fixed, since per-key concurrent processing is new concurrency architecture, not a bug fix.
