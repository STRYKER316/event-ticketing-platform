...
# This service's own DTO validation boundary for a Kafka payload, same rule as an API request body — event-service's own rejection doesn't cover this side.
...
class EventUpsertedMessage(BaseModel):
    ...
    # AwareDatetime, not bare: start_time is compared against datetime.now(timezone.utc) for the cancellation cutoff (§22 amendment #2).
...
class NotificationAction(enum.Enum):
    # PAYMENT_CONFIRMED and REFUND_FAILED are payment-service's own producer-side members (§7 point 3), not sent from here.
