...
class RedisHoldStrategy(TicketHoldStrategy):
    ...
    entire correctness mechanism; there is no sweep (see hold_sweep.py,
    which only exists for the cron strategy) and no Postgres write to keep
    in sync at acquire time. Consequence, documented rather than hidden: an
    abandoned hold under this strategy leaves tickets.status at AVAILABLE
    for its whole lifetime — nothing here ever claims otherwise. Anything
    needing live hold status must call is_held() on whichever strategy is
    actually active, never read tickets.status directly, since that column
    only reflects hold state under the cron strategy."""
